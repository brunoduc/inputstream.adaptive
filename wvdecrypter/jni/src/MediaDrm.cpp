...
std::map<std::string, std::string> CJNIMediaDrm::queryKeyStatus(const std::vector<char> &sessionId)
{
  if (CJNIBase::GetSDKVersion() >= 23)
  {
    ...
    {
      jhobject next = call_method<jhobject>(iterator, "next", "()Ljava/lang/Object;");
      std::string key = jcast<std::string>(call_method<jhstring>(next, "getKey", "()Ljava/lang/String;");
      std::string value = jcast<std::string>(call_method<jhstring>(next, "getValue", "()Ljava/lang/String;");
      result[key] = value;
    }
    return result;
  }
...
int CJNIMediaDrm::getSecurityLevel(const std::vector<char> &sessionId)
{
...
int CJNIMediaDrm::getMaxSecurityLevel()
{
  if (CJNIBase::GetSDKVersion() >= 28)
  {
